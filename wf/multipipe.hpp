...
private:
                                  ...
                                  bool isDestGPU=false) const
    {
        ...
        else if (_routing_mode == Routing_Mode_t::HYBRID_JOIN) { // HJ
            return new HybridJoin_Emitter<decltype(_key_extr)>(_key_extr, _num_dests, hybrid_parallelism, _outputBatchSize);
        }
        else {
            abort();
        }
    ...
    {
        if constexpr (source_t::op_type != op_type_t::SOURCE) {
            std::cerr << RED << "WindFlow Error: add_source invoked with a non-source operator" << DEFAULT_COLOR << std::endl;
            exit(EXIT_FAILURE);         
        }
    ...
    void add_operator(operator_t &_operator, ordering_mode_t _ordering_mode)
    {
        ...
        if (fromSplitting && last == nullptr) { // Case 1: first operator added after splitting
            ...
            Basic_Emitter *emitter = create_emitter<decltype(_operator.getKeyExtractor()), isDestGPUType>(_operator.getKeyExtractor(), _operator.getInputRoutingMode(), (_operator.replicas).size(), outputBatchSize, 0, isSourceGPU, isDestGPU);
            ...
            if ((n1 == n2) && (_operator.getInputRoutingMode() == Routing_Mode_t::FORWARD)) { // Case 2: direct connection
                ...
                (localOpList.back())->setEmitter(create_emitter<decltype(_operator.getKeyExtractor()), isDestGPUType>(_operator.getKeyExtractor(), _operator.getInputRoutingMode(), 1, outputBatchSize, 0, isSourceGPU, isDestGPU));
            }
                ...
                size_t hybrid_parallelism = 1;
                if(_operator.getType() == "Interval_Join_HP"){
                    hybrid_parallelism = _operator.getHybridParallelism();
                }
                for (auto *op: lastOps) {
                ...
                for (auto *op: lastOps) { // set the emitter to the last operator(s) in the MultiPipe
                    ...
                    op->setEmitter(create_emitter<decltype(_operator.getKeyExtractor()), isDestGPUType>(_operator.getKeyExtractor(), _operator.getInputRoutingMode(), _operator.getParallelism(), outputBatchSize, hybrid_parallelism, isSourceGPU, isDestGPU));
                }
...
public:
...
#if defined (WF_TRACING_ENABLED)
            gv_add_vertex(_op.getType() + " (" + std::to_string(_op.plq_parallelism) + "," + std::to_string(_op.wlq_parallelism) + ")", _op.getName(), true, false, _op.getInputRoutingMode());
#endif
        }
...
#if defined (WF_TRACING_ENABLED)
            gv_add_vertex(_op.getType() + " (" + std::to_string(_op.map_parallelism) + "," + std::to_string(_op.reduce_parallelism) + ")", _op.getName(), true, false, _op.getInputRoutingMode());
#endif
        }
        else if constexpr (op_t::op_type == op_type_t::WIN_GPU) {
    ...
    MultiPipe &chain(const op_t &_op)
    {
        ...
        else if constexpr (op_t::op_type == op_type_t::BASIC) {
    ...
    {
        if constexpr (sink_t::op_type != op_type_t::SINK) {
            std::cerr << RED << "WindFlow Error: add_sink invoked with a non-sink operator" << DEFAULT_COLOR << std::endl;
            exit(EXIT_FAILURE);         
        }
    ...
    {
        if constexpr (sink_t::op_type != op_type_t::SINK) {
            std::cerr << RED << "WindFlow Error: chain_sink invoked with a non-sink operator" << DEFAULT_COLOR << std::endl;
            exit(EXIT_FAILURE);         
        }
        auto *copied_sink = new sink_t(_sink); // create a copy of the operator
        copied_sink->setExecutionMode(execution_mode);
    ...
    MultiPipe &split(splitting_func_t _splitting_func, size_t _cardinality)
    {
        ...
        if (localOpList.back()->isGPUOperator()) { // split cannot be used after a GPU operator
            std::cerr << RED << "WindFlow Error: GPU operator before a split, use split_gpu() instead" << DEFAULT_COLOR << std::endl;
            exit(EXIT_FAILURE);
        }
    ...
    MultiPipe &split_gpu(size_t _cardinality)
    {
        ...
        if (!localOpList.back()->isGPUOperator()) { // split_gpu cannot be used after a CPU operator
            std::cerr << RED << "WindFlow Error: non-GPU operator before a split_gpu, use split() instead" << DEFAULT_COLOR << std::endl;
            exit(EXIT_FAILURE);
        }
        splittingBranches = _cardinality;
