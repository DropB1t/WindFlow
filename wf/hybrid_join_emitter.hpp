...
private:
    ...
    std::vector<Batch_CPU_t<tuple_t> *> batches_output; // vector of pointers to the output batches one per range of destinations (meaningful if size > 0)
...
public:
    ...
    void routing(Single_t<tuple_t> *_output, ff::ff_monode *_node)
    {
        ...
        size_t i = hashcode % num_dests; // compute the initial destination identifier
        size_t sends = hybrid_degree;
        while(sends > 0) {
            assert(last_sent_wms[i] <= _output->getWatermark()); // sanity check
            last_sent_wms[i] = _output->getWatermark(); // save the last watermark emitted to this destination
                         ...
                         ff::ff_monode *_node)
    {
        ...
        size_t hashcode = std::hash<key_t>()(key); // compute the hashcode of the key
        size_t initial_id = hashcode % num_dests; // compute the initial destination identifier
        if (batches_output[initial_id] == nullptr) {
            batches_output[initial_id] = allocateBatch_CPU_t<tuple_t>(size, this->queue);
        }
        auto batch_output = batches_output[initial_id];
        batch_output->addTuple(std::move(_tuple), _timestamp, _watermark);
        if (batch_output->getSize() == size) { // batch is ready to be sent
            (batch_output->delete_counter).fetch_add(hybrid_degree-1);
            ...
            size_t i = initial_id; // compute the initial destination identifier
            size_t sends = hybrid_degree;
            while(sends > 0) {
                if (!useTreeMode) { // real send
            ...
            batches_output[initial_id] = nullptr;
        }
    }
        ...
        if (size > 0) { // only batching
            for (size_t i=0; i<num_dests; i++) {
                if (batches_output[i] != nullptr) {
                    ...
                    size_t j = i; // compute the initial destination identifier
                    size_t sends = hybrid_degree;
                    while(sends > 0) {
                        if (!useTreeMode) { // real send
