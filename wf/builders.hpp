...
private:
    ...
    uint64_t win_size=0; // window size expressed in time unit
    uint64_t sliding_len=0; // sliding length expressed in time unit
...
public:
     ...
     *  @param _sliding_length sliding length expressed in time unit
     *  @return a reference to the current object
     */ 
    ...
    {
        if (!isKeyBySet) {
            std::cerr << RED << "WindFlow Error: Window_Join with key parallelism mode requires a key extractor" << DEFAULT_COLOR << std::endl;
            exit(EXIT_FAILURE);
        }
        if (join_mode != Join_Mode_t::NONE) {
    ...
    {
        if (!isKeyBySet) {
            std::cerr << RED << "WindFlow Error: Window_Join with data parallelism mode requires a key extractor" << DEFAULT_COLOR << std::endl;
            exit(EXIT_FAILURE);
        }
        if (join_mode != Join_Mode_t::NONE) {
    ...
    {
        if (!isKeyBySet) {
            std::cerr << RED << "WindFlow Error: Window_Join with hybrid parallelism mode requires a key extractor" << DEFAULT_COLOR << std::endl;
            exit(EXIT_FAILURE);
        }
        if (join_mode != Join_Mode_t::NONE) {
            std::cerr << RED << "WindFlow Error: wrong use of withHPMode() in the Window_Join_Builder, you can specify only one mode per join operator" << DEFAULT_COLOR << std::endl;
            exit(EXIT_FAILURE);
        }
        hybrid_parallelism = _hybrid_parallelism;
        input_routing_mode = Routing_Mode_t::HYBRID_JOIN;
