...
import join.NestedPayload;

// FileSource class
public class FileSource extends RichParallelSourceFunction<Tuple7<Integer, Integer, Long, String, Integer, Integer, NestedPayload>> {
    ...
    // run method
    @Override
    public void run(final SourceContext<Tuple7<Integer, Integer, Long, String, Integer, Integer, NestedPayload>> ctx) throws Exception {
        ...
        // while(true) {
            ...
            if (mode == 0) { // standard
	            for (Integer dstId: targets) {
	            	NestedPayload payload = new NestedPayload(payload_size);
	            	ctx.collectWithTimestamp(new Tuple7<>(tuple.key, tuple.value, processing_time, streamTag, sourceId, dstId, payload), ts);
	            }
            }
            else { // custom
            	NestedPayload payload = new NestedPayload(payload_size);
	           	ctx.collectWithTimestamp(new Tuple7<>(tuple.key, tuple.value, processing_time, streamTag, sourceId, -1, payload), ts);
            }
            generated++;
            index++;
        ...
        if (mode == 0) { // standard
	        ...
	        for (int i=0; i<numJoiners; i++) {
	        	NestedPayload payload = new NestedPayload(1);
	        	ctx.collectWithTimestamp(new Tuple7<>(-1, 0, 0L, streamTag, sourceId, i, payload), -1);
	        }
        }
        else { // custom
	        NestedPayload payload = new NestedPayload(1);
        	ctx.collectWithTimestamp(new Tuple7<>(-1, 0, 0L, streamTag, sourceId, -1, payload), -1);
        }
