...
public class RawIntervalJoin extends AbstractStreamOperator<Tuple3<Integer, Integer, Long>>
                             implements OneInputStreamOperator<Tuple7<Integer, Integer, Long, String, Integer, Integer, NestedPayload>, Tuple3<Integer, Integer, Long>> {
    private static final Logger LOG = LoggerFactory.getLogger(RawIntervalJoin.class);
    private int joinerId;
    private int numJoiners;
    private transient Map<Integer, Long> counters;
    ...
    private transient HashMap<Integer, PriorityQueue<TimerEntry>> timerHeap;

    // constructor
    public RawIntervalJoin(long _lowerBound, long _upperBound, Map<Integer, List<Integer>> _keyToJoiners) {
        ...
        this.keyToJoiners = _keyToJoiners;
    }
    ...
    @Override
    public void processElement(StreamRecord<Tuple7<Integer, Integer, Long, String, Integer, Integer, NestedPayload>> element) {
        final Tuple7<Integer, Integer, Long, String, Integer, Integer, NestedPayload> tuple = element.getValue();
        final long evtTs = element.getTimestamp();

        if (tuple.f2 == -1) {
        	processKeyedWatermark(tuple.f0, evtTs);
        	return;
        }
        else {
    ...
    // processKeyedWatermark method
    public void processKeyedWatermark(Integer key, long wm) {
    	if (timerHeap.get(key) == null) {
    		return;
    	}
    	else {
    		while (!timerHeap.get(key).isEmpty() && timerHeap.get(key).peek().timestamp <= wm) {
       			TimerEntry entry = timerHeap.get(key).poll();
        		fireCleanup(key, entry);
    		}
    	}
    }
    ...
    private void processJoinOrange(BufferEntry bt) {
        final Tuple7<Integer, Integer, Long, String, Integer, Integer, NestedPayload> t = bt.tuple;
        orangeBuffer.computeIfAbsent(t.f0, k -> new TreeMap<>());
        greenBuffer.computeIfAbsent(t.f0, k -> new TreeMap<>());
        counters.computeIfAbsent(t.f0, k -> 0L);
        timerHeap.computeIfAbsent(t.f0, k -> new PriorityQueue<>());
    ...
    private void processJoin(
        ...
        long c = (counters.get(t.f0)).longValue() + 1;
		List<Integer> joiners = keyToJoiners.get(t.f0);
		int index = (int) (c % joiners.size());
		if (joiners.get(index) == joinerId) {
    	...
    	long cleanupTime = (relativeUpperBound > 0L) ? ts + relativeUpperBound : ts;
   		if (isOrange) {
   			timerHeap.get(t.f0).add(new TimerEntry(cleanupTime, "orange"));
   		}
   		else {
   			timerHeap.get(t.f0).add(new TimerEntry(cleanupTime, "green"));
   		}
    }
    ...
    // fireCleanup method
    private void fireCleanup(Integer key, TimerEntry entry) {
    	if ((entry.type).equals("orange")) {
    		NavigableMap<Long, List<BufferEntry>> buffer = orangeBuffer.get(key);
    		if (buffer != null) {
    		...
    		NavigableMap<Long, List<BufferEntry>> buffer = greenBuffer.get(key);
    		if (buffer != null) {
    ...
    // joinEmit method
    private void joinEmit(Tuple7<Integer, Integer, Long, String, Integer, Integer, NestedPayload> left,
                          Tuple7<Integer, Integer, Long, String, Integer, Integer, NestedPayload> right,
                          long leftTs,
                          long rightTs) {
	...
	public static class TimerEntry implements Comparable<TimerEntry> {
	    ...
	    // constructor
	    TimerEntry(long _timestamp, String _type) {
