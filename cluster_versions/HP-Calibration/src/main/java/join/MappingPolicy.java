...
public class MappingPolicy {
    ...
    // computeProbabilities method (new version for the calibration)
	static void computeProbabilities(Map<Integer, Integer> freqKeys, List<Double> probs) {
    	// first, compute the total number of tuples
    	int totalTuples = 0;
    	for (int count: freqKeys.values()) {
        	totalTuples += count;
    	}
   		// clear the probs list in case it has existing values
    	probs.clear();
	    // compute the probability for each key from 0 to max key
	    int maxKey = freqKeys.keySet().stream().max(Integer::compareTo).orElse(0);
	    for (int i=0; i<=maxKey; i++) {
	        int count = freqKeys.getOrDefault(i, 0);
	        double probability = totalTuples > 0 ? (double) count / totalTuples : 0.0;
    ...
    static void assignKeys(int numJoiners,
        ...
        // FIRST PASS (here is different, and it is based on the initial assignment before calibration)
        for (KeyD keyD: sortedKeys) {
        	int destJoiner = keyD.key % numJoiners;
        	Joiner j = joiners.get(destJoiner);
        	j.assignedKeys.add(keyD.key);
        	j.load += keyD.prob;
        	splitDegrees.set(keyD.key, 1);
            keyToJoiners.computeIfAbsent(keyD.key, k -> new ArrayList<>()).add(destJoiner);
        }

        if ((balancing(joiners) <= threshold) && allLoaded(joiners)) {
            LOG.info("[Router] STOP FIRST PASS -> final balancing is: " + balancing(joiners) + ", threshold was: " + threshold + ", average splitting degree: " + averageSplitting(splitDegrees));
            return;
        }
        ...
        LOG.info("[Router] STOP SECOND PASS -> final balancing is: " + balancing(joiners) +
