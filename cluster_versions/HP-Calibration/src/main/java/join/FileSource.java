...
// FileSource class
public class FileSource extends RichParallelSourceFunction<Tuple7<Integer, Integer, Long, String, Integer, Integer, Integer>> {
    ...
    public void open(Configuration parameters) throws Exception {
        ...
        parSource = getRuntimeContext().getNumberOfParallelSubtasks();
        if (streamTag.equals("orange")) {
            dstId = sourceId;
        }
        else {
            dstId = parSource + sourceId;
        }
    }

    // run method
    @Override
    public void run(final SourceContext<Tuple7<Integer, Integer, Long, String, Integer, Integer, Integer>> ctx) throws Exception {
        ...
        // while(true) {
            ...
            ctx.collectWithTimestamp(new Tuple7<>(tuple.key, tuple.value, -1L, streamTag, sourceId, dstId, -1), ts);
            generated++;
            index++;
        ...
        ctx.collectWithTimestamp(new Tuple7<>(-1, -1, -1L, streamTag, sourceId, dstId, -1), -1);
